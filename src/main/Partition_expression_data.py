...
if __name__ == "__main__":
        ...
        parser.add_argument("-np", "--no_pca", action="store_true",
        help = "Use this to skip Standardization and PCA step. Will read from pregenerated PCA data in outputdir." )

        parser.add_argument("-nk", "--no_kmeans", action="store_true",
        help = "Use this to skip kmeans clustering. Will read from pre-generated SCs and kmeans assignments in outputdir." )
        ...
        pc_no = args.pc_no
        no_pca = args.no_pca
        no_kmeans = args.no_kmeans
        if no_kmeans:
                print(f"reading k-means clustering data from output dir...")
                k_cluster_assignment_dict = read_write.load_pickle(os.path.join(sub_outdir,"k_cluster_assignment_dict.pkl"))
                silhouette_coefficients = read_write.load_pickle(os.path.join(sub_outdir, "silhouette_coefficients.pkl"))
        else:
                if no_pca:
                        print("Skiping standardization and PCA. Reading PCA data from output dir directly.")
                        pca_data = read_write.load_pickle(os.path.join(sub_outdir, "PCA_data.pkl"))
                        pca_data = PCA.subset_pca_data(pca_data, n_pcs=1000)
                        print("This is how the PCA data look like:")
                        print(pca_data) # rmb to remove
                else:
                ...
                k_cluster_assignment_dict , silhouette_coefficients, centroids_dict = kmeans.iterate_over_krange(pca_data, k_list ,k_cluster_assignment_dict_path , silhouette_coefficients_dict_path, mode = "kmeans",randomstate=42)
